...
constructor TSD.Create;
begin
  inherited;
  mNLOType := nlSD;
end;

function TSD.Name: string;
begin
  Name := 'SD';
end;

function TSD.SpectrumMultiplier: double;
begin
  SpectrumMultiplier := 1.0;
end;

procedure TSD.BasicField(pEsig: TSignalField; pEk: TEField);
var
...
begin
  ...
  // We acutally use the Gate Field to get the basic Field in SD
...
procedure TSD.GateField(pEsig: TSignalField; pEk: TEField);
var
  w, N, tau: integer;
  phi, tInt: double;
begin
...
procedure TSD.MakeSignalField(pEk: TEField; pEsig: TSignalField);
var
...
procedure TSD.MakeXSignalField(pEk, pERef: TEField; pEsig: TSignalField);
var
	tau, t, t0, N, tp: integer;
  u, v: double;
begin
...
function TSD.Z(pEx: PArray; pEsig: TSignalField): double;
var
...
procedure TSD.ZDerivative(pEk: TEField; pEsig: TSignalField; Derivs: PArray);
var
...
function TSD.XFrogZ(pEx: PArray; pERef: TEField; pEsig: TSignalField): double;
var
...
procedure TSD.XFrogZDerivative(pEk, pERef: TEField; pEsig: TSignalField; Derivs: PArray);
var
  t, tp, tau, N, N2, vr, vi: integer;
  sigr, sigi, brakr, braki, re, im, e2r, e2i: double;
begin
...
procedure TSD.GDerivative(pEk: TEField; pEsig: TSignalField; pFrogI: TFrogTrace; Derivs: PArray);
var
...
function TSD.NeedSpectrum: Boolean;
begin
  NeedSpectrum := True;
end;
function TSD.NeedAutocorrelation: Boolean;
begin
  NeedAutocorrelation := False;
end;
function TSD.NeedSHGSpectrum: Boolean;
begin
  NeedSHGSpectrum := True;
end;

function TSD.CalculateMarginal(pSpec: TSpectrum; pAuto: TAutocorrelation; pSHGSpec: TSpectrum): TMarginal;
var
...
begin
  ...
  try
    ...
    for i := 0 to N - 1 do
    begin
      Spectrum.Re^[i] := tempSpec.Intensity^[N - 1 - i];
      Spectrum.Im^[i] := 0.0;
    end;
...
function TSD.XTestLam(frogLam, refLam: double): double;
var
  denom: double;
begin
  denom := 2.0/refLam - 1.0/frogLam;
...
function TSD.XFrogLam(refLam, testLam: double): double;
var
   denom: double;
begin
  denom := 2.0/refLam - 1.0/testLam;
